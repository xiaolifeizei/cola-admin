...
public class ServiceProviderDubboFilter implements Filter,BaseFilter.Listener {
    @Override
    public Result invoke(Invoker<?> invoker, Invocation invocation) throws RpcException {
        ...
        try {
            // 调用消费者方法
            log.trace("提供者执行->" + invoker.getInterface() + "." + invocation.getMethodName());
    ...
    public void onError(Throwable t, Invoker<?> invoker, Invocation invocation) {
        DubboUtil.clearInvokeName();
        DubboUtil.clearUser();
        log.trace("提供者执行出错了:" + t.getMessage());
        log.trace("------------提供者执行出错了，清除数据------------");
    }
}
